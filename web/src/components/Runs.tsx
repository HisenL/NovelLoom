...
export function Runs({ projectId }: { projectId: string }) {
  ...
  const start = async () => { await request(`/api/projects/${projectId}/workflow/start`, { method: 'POST' }); setNotice('已推进至下一个人工审核门。'); await load() }
  ...
  return <section><div className="toolbar"><div><span className="kicker">DURABLE WORKFLOW</span><h2>运行、人工门与恢复</h2></div><button onClick={() => void start().catch((e) => setNotice(e.message))}>启动规划流程</button></div>{notice && <div className="notice">{notice}</div>}
